    ...
    for (tab, col) in self.tabs.zip(range(self.tabs.len())) {
      ...
      if type(tab) == int and tab > 0 and tab <= self.frets-amount {
        place(
          ...
          circle(radius: radius, stroke: none, fill: self.hold-color)
        )
        continue
      }
    }
    ...
    for (fret, start, end, ..) in self.capos {
      ...
      place(
        ...
        line(
          ...
          stroke: (paint: self.hold-color, thickness: 3.4pt * self.scale, cap: "round")
        )
      )
    }
  }
...
#let chart-chord(
  ...
  /// [PATCH] Sets the fill color of the finger dots and barres. *Optional*.
  /// -> color
  hold-color: black,
